...
void	ftInitBlocks(Game *game, SquareProps **blocks, EnvItems **envItems)
{
	...
	envItems[0] = new EnvItems((Vector2){0, 400}, (Vector2){1000, 200}, 1, GRAY, tmp, 1, "Platform1");
	envItems[1] = new EnvItems((Vector2){300, 150}, (Vector2){400, 10}, 1, GRAY, tmp, 2, "Platform2");
	envItems[2] = new EnvItems((Vector2){250, 250}, (Vector2){100, 10}, 1, GRAY, tmp, 3, "Platform3");
	envItems[3] = new EnvItems((Vector2){650, 250}, (Vector2){100, 10}, 1, GRAY, tmp, 4, "Platform4");
	game->nbrEnvi = 4;
}
			...
			BeginMode2D(allCameras->camera00.camera);
				...
				else if (menu->ftReturnStart() == 2) // Main loop
				{
					...
					else if (game->ctMode == -1)
					{
						...
						{
							tmpEnvItems[i] = new EnvItems(*envItems[i]);
						}
						for (int i = 0; i < game->nbrSquare; i++)
						{
							tmpBlocks[i] = new SquareProps(*blocks[i]);
						}
...
void	ftControlItems(Game *game, Player *player, EnvItems **envItems, SquareProps **blocks)
{
	...
	else if (game->selected2D.type == 3) // Platforms
	{
		...
		if (IsMouseButtonDown(MOUSE_BUTTON_LEFT) && game->mouse.pos.x < game->screenWidth - 300 && game->mouse.pos.y > 40)
		{
			game->selected2D.item->ftMovePos({(-(forMove.x / game->mouse.camZoom)),
				(-(forMove.y / game->mouse.camZoom))});
		}
	}
}
