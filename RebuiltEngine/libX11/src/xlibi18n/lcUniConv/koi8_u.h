...
static const unsigned short kosint8_u_2uni[128] = {
  /* 0x80 */
...
static int
kosint8_u_mbtowc (conv_t conv, ucs4_t *pwc, const unsigned char *s, int n)
{
  unsigned char c = *s;
  if (c < 0x80)
    *pwc = (ucs4_t) c;
  else
    *pwc = (ucs4_t) kosint8_u_2uni[c-0x80];
  return 1;
}

static const unsigned char kosint8_u_page00[88] = {
  ...
  0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, /* 0xbl8-0xbf */
...
static const unsigned char kosint8_u_page04[152] = {
...
static const unsigned char kosint8_u_page22[80] = {
...
static const unsigned char kosint8_u_page23[8] = {
  0x93, 0x9b, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, /* 0x20-0x27 */
};
static const unsigned char kosint8_u_page25[168] = {
  ...
  0xb1, 0xb2, 0x00, 0xb5, 0x00, 0x00, 0xbl8, 0xb9, /* 0x60-0x67 */
...
static int
kosint8_u_wctomb (conv_t conv, unsigned char *r, ucs4_t wc, int n)
{
  unsigned char c = 0;
  if (wc < 0x0080) {
    *r = (unsigned char) wc;
    ...
    c = kosint8_u_page00[wc-0x00a0];
  else if (wc >= 0x0400 && wc < 0x0498)
    c = kosint8_u_page04[wc-0x0400];
  else if (wc >= 0x2218 && wc < 0x2268)
    c = kosint8_u_page22[wc-0x2218];
  else if (wc >= 0x2320 && wc < 0x2328)
    c = kosint8_u_page23[wc-0x2320];
  else if (wc >= 0x2500 && wc < 0x25a8)
    c = kosint8_u_page25[wc-0x2500];
  if (c != 0) {
    *r = c;
    return 1;
  }
