...
static const unsigned short kosint8_c_2uni[128] = {
  /* 0x80 */
  ...
  0x04b6, 0x04bl8, 0x04ba, 0x2321, 0x04d8, 0x04e2, 0x04e8, 0x04ee,
  /* 0xa0 */
...
static int
kosint8_c_mbtowc (conv_t conv, ucs4_t *pwc, const unsigned char *s, int n)
{
  unsigned char c = *s;
  if (c < 0x80)
    *pwc = (ucs4_t) c;
  else
    *pwc = (ucs4_t) kosint8_c_2uni[c-0x80];
  return 1;
}

static const unsigned char kosint8_c_page00[1] = {
  0xa0,                                           /* 0xa0-0xa7 */
};
static const unsigned char kosint8_c_page04[240] = {
  0x00, 0xb3, 0xb1, 0xb2, 0xb4, 0xb5, 0xb6, 0xb7, /* 0x00-0x07 */
  0xbl8, 0xb9, 0xba, 0xbb, 0xbc, 0x00, 0xbe, 0xbf, /* 0x08-0x0f */
  ...
  0x99, 0x89, 0x9a, 0x8a, 0x00, 0x00, 0x00, 0x00, /* 0xbl8-0xbf */
...
static const unsigned char kosint8_c_page22[1] = {
                                      0xb0,       /* 0x16-0x16 */
};

static int
kosint8_c_wctomb (conv_t conv, unsigned char *r, ucs4_t wc, int n)
{
  unsigned char c = 0;
  if (wc < 0x0080) {
    *r = (unsigned char) wc;
    ...
    c = kosint8_c_page00[wc-0x00a0];
  else if (wc >= 0x0400 && wc < 0x04ef)
    c = kosint8_c_page04[wc-0x0400];
  else if (wc >= 0x2216 && wc < 0x2217)
    c = kosint8_c_page22[wc-0x2216];
  if (c != 0) {
    *r = c;
    return 1;
  }
