...
void vulkan_device_query_swapchain_support(
    ...
    if (out_support_info->format_count != 0) {
        if (!out_support_info->formats) {
            out_support_info->formats = (VkSurfaceFormatKHR *)kallocate(sizeof(VkSurfaceFormatKHR) * out_support_info->format_count, DE_MEMORY_TAG_RENDERER);
        }
        VK_CHECK(vkGetPhysicalDeviceSurfaceFormatsKHR(
            physical_device,
            surface,
    ...
    if (out_support_info->present_mode_count != 0) {
        if (!out_support_info->present_modes) {
            out_support_info->present_modes = (VkPresentModeKHR *)kallocate(sizeof(VkPresentModeKHR) * out_support_info->present_mode_count, DE_MEMORY_TAG_RENDERER);
        }
        VK_CHECK(vkGetPhysicalDeviceSurfacePresentModesKHR(
            physical_device,
            surface,
    ...
    if (
        ...
        if (out_swapchain_support->format_count < 1 || out_swapchain_support->present_mode_count < 1) {
            if (out_swapchain_support->formats) {
                kfree(out_swapchain_support->formats, sizeof(VkSurfaceFormatKHR) * out_swapchain_support->format_count, DE_MEMORY_TAG_RENDERER);
            }
            if (out_swapchain_support->present_modes) {
                kfree(out_swapchain_support->present_modes, sizeof(VkPresentModeKHR) * out_swapchain_support->present_mode_count, DE_MEMORY_TAG_RENDERER);
            }
        ...
        if (requirements->device_extension_names) {
            ...
            if (available_extension_count != 0) {
                available_extensions = (VkExtensionProperties *)kallocate(sizeof(VkExtensionProperties) * available_extension_count, DE_MEMORY_TAG_RENDERER);
                VK_CHECK(vkEnumerateDeviceExtensionProperties(
                    device,
                    0,
                    &available_extension_count,
                ...
                for (uint32 i = 0; i < required_extension_count; ++i) {
                    ...
                    if (!found) {
                        ...
                        kfree(available_extensions, sizeof(VkExtensionProperties) * available_extension_count, DE_MEMORY_TAG_RENDERER);
                        return false;
                    }
                }
            }
